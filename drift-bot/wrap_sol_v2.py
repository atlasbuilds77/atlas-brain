...
async def main():
    ...
    # Check if ATA exists and get current WSOL balance
    ...
    # Send transaction (sync_native happens automatically when SOL is sent to token account)
    recent_blockhash = (await connection.get_latest_blockhash()).value.blockhash
    from solders.transaction import Transaction
    tx = Transaction(
        fee_payer=keypair.pubkey(),
    ...
    if account_info.value:
        ...
        print(f"New WSOL Balance: {wsol_balance:.6f} WSOL")
