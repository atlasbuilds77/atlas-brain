#!/usr/bin/env python3
"""
Kalshi Position Checker - Fixed with API Key Auth + Finalized Markets
Check all open positions including finalized markets
"""
import os
import sys
import requests
from kalshi_python import Configuration, KalshiClient
from collections import defaultdict

# Credentials
API_KEY_ID = "0007b9f0-89c9-42b4-93bd-f98fbf1596b8"
PRIVATE_KEY_PATH = "/Users/atlasbuilds/.clawdbot/credentials/kalshi/private_key.pem"
API_HOST = "https://api.elections.kalshi.com/trade-api/v2"

def get_client():
    """Initialize Kalshi API client"""
    if not os.path.exists(PRIVATE_KEY_PATH):
        print(f"❌ Private key not found at {PRIVATE_KEY_PATH}")
        sys.exit(1)
    
    with open(PRIVATE_KEY_PATH, "r") as f:
        private_key = f.read()
    
    config = Configuration(host=API_HOST)
    config.api_key_id = API_KEY_ID
    config.private_key_pem = private_key
    
    return KalshiClient(config)

def get_market_raw(ticker):
    """Get market info using raw API to handle finalized markets"""
    try:
        # Use raw requests to bypass SDK validation
        url = f"{API_HOST}/markets/{ticker}"
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            return data.get('market', {})
    except:
        pass
    return None

def get_positions_from_fills(client):
    """Calculate positions from fills"""
    fills_resp = client.get_fills(limit=200)
    
    positions = defaultdict(lambda: {'count': 0, 'fills': []})
    
    if hasattr(fills_resp, 'fills') and fills_resp.fills:
        for fill in fills_resp.fills:
            ticker = fill.ticker if hasattr(fill, 'ticker') else None
            side = fill.side if hasattr(fill, 'side') else None
            count = fill.count if hasattr(fill, 'count') else 0
            
            if not ticker or not side:
                continue
            
            positions[ticker]['fills'].append({
                'side': side,
                'count': count,
                'created': fill.created_time if hasattr(fill, 'created_time') else None
            })
            
            # Calculate net position
            if side.lower() == 'yes':
                positions[ticker]['count'] += count
            elif side.lower() == 'no':
                positions[ticker]['count'] -= count
    
    return {k: v for k, v in positions.items() if v['count'] != 0}

def main():
    print("=" * 70)
    print("KALSHI POSITION CHECKER")
    print("=" * 70)
    
    try:
        client = get_client()
        print("\n✅ Connected to Kalshi API")
        
        # Get balance
        balance_resp = client.get_balance()
        balance = balance_resp.balance if hasattr(balance_resp, 'balance') else 0
        print(f"💰 Cash Balance: ${balance / 100:.2f}\n")
        
        # Get positions from fills
        active_positions = get_positions_from_fills(client)
        
        print(f"📊 ACTIVE POSITIONS ({len(active_positions)}):\n")
        
        if not active_positions:
            print("   No active positions")
            print("\n" + "=" * 70)
            return
        
        total_position_value = 0
        total_max_payout = 0
        
        for ticker, pos_data in sorted(active_positions.items()):
            net_count = pos_data['count']
            side = 'YES' if net_count > 0 else 'NO'
            abs_count = abs(net_count)
            
            # Try SDK first, fall back to raw API
            market_data = None
            try:
                market_resp = client.get_market(ticker=ticker)
                if hasattr(market_resp, 'market'):
                    m = market_resp.market
                    market_data = {
                        'title': m.title if hasattr(m, 'title') else ticker,
                        'status': m.status if hasattr(m, 'status') else 'unknown',
                        'yes_bid': m.yes_bid if hasattr(m, 'yes_bid') else 0,
                        'yes_ask': m.yes_ask if hasattr(m, 'yes_ask') else 0,
                        'no_bid': m.no_bid if hasattr(m, 'no_bid') else 0,
                        'no_ask': m.no_ask if hasattr(m, 'no_ask') else 0,
                        'close_time': str(m.close_time) if hasattr(m, 'close_time') else 'Unknown'
                    }
            except:
                # Fall back to raw API for finalized markets
                raw_market = get_market_raw(ticker)
                if raw_market:
                    market_data = {
                        'title': raw_market.get('title', ticker),
                        'status': raw_market.get('status', 'unknown'),
                        'yes_bid': raw_market.get('yes_bid', 0),
                        'yes_ask': raw_market.get('yes_ask', 0),
                        'no_bid': raw_market.get('no_bid', 0),
                        'no_ask': raw_market.get('no_ask', 0),
                        'close_time': raw_market.get('close_time', 'Unknown'),
                        'result': raw_market.get('result', None)
                    }
            
            if market_data:
                title = market_data['title']
                status = market_data['status']
                yes_bid = market_data['yes_bid']
                yes_ask = market_data['yes_ask']
                no_bid = market_data['no_bid']
                no_ask = market_data['no_ask']
                close_time = market_data['close_time']
                result = market_data.get('result')
                
                # Calculate values
                current_price = yes_bid if side == 'YES' else no_bid
                current_value = abs_count * current_price / 100
                max_payout = abs_count  # Max payout is $1 per contract
                
                total_position_value += current_value
                total_max_payout += max_payout
                
                # Status emoji
                if status in ['finalized', 'settled']:
                    emoji = '✅' if result == side else '❌'
                elif status == 'closed':
                    emoji = '🔒'
                else:
                    emoji = '📈'
                
                print(f"{emoji} {ticker}")
                print(f"   Market: {title}")
                print(f"   Position: {side} × {abs_count} contracts")
                print(f"   Current Bid: {current_price}¢")
                print(f"   Current Value: ${current_value:.2f}")
                print(f"   Max Payout: ${max_payout:.2f}")
                print(f"   Status: {status}")
                
                if result:
                    print(f"   Result: {result} {'✅ WIN' if result == side else '❌ LOSS'}")
                
                if status == 'active':
                    print(f"   Closes: {close_time}")
                    print(f"   Spread: YES {yes_bid}¢/{yes_ask}¢  NO {no_bid}¢/{no_ask}¢")
                
                print()
            else:
                print(f"❓ {ticker}: {side} {abs_count} contracts (could not fetch market data)")
                print()
        
        print("-" * 70)
        print(f"💵 CASH BALANCE:         ${balance / 100:>10.2f}")
        print(f"📊 POSITION VALUE:       ${total_position_value:>10.2f}")
        print(f"💰 MAX PAYOUT:           ${total_max_payout:>10.2f}")
        print(f"💎 TOTAL ACCOUNT VALUE:  ${total_position_value + balance / 100:>10.2f}")
        print("=" * 70)
        
    except Exception as e:
        print(f"\n❌ Error: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)

if __name__ == "__main__":
    main()
