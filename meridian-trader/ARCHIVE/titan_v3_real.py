...
def find_sweep_reclaim(bars: List[dict], level: float, level_type: str) -> Optional[dict]:
    ...
    - No reclaim in 5 min = Breakdown, not a sweep → SKIP IT
    
    Evidence:
    - Feb 13: Sweep + instant reclaim (0 min) → Worked
    - Feb 11: Sweep + no reclaim (30+ min) → Would have been a loser
    """
    MAX_RECLAIM_BARS = 5  # 5 minutes max for reclaim
    
    for i, bar in enumerate(bars):
        if level_type == "high" and bar['h'] > level:
            for j in range(i + 1, min(i + 1 + MAX_RECLAIM_BARS, len(bars))):
                if bars[j]['c'] < level:
                    reclaim_time_mins = j - i
                    return {
                        "sweep_idx": i,
                        "sweep_time": bar['time_et'],
                        "reclaim_idx": j,
                        "reclaim_time": bars[j]['time_et'],
                        "reclaim_mins": reclaim_time_mins,
                        "entry_price": bars[j]['c'],
                        "direction": "SHORT"
                    }
            return None  # No reclaim within 5 min = SKIP
...
def backtest_day(symbol: str, date: str) -> Optional[dict]:
    ...
    # Find targets - next significant clusters in the direction
    entry = best_setup['entry_price']
    direction = best_setup['direction']
    
    if direction == "SHORT":
        # Target lower clusters
        targets = sorted([c['price'] for c in all_clusters 
                         if c['level_type'] == 'low' and c['price'] < entry], reverse=True)
    else:
        # Target higher clusters
        targets = sorted([c['price'] for c in all_clusters 
                         if c['level_type'] == 'high' and c['price'] > entry])
    
    target1 = targets[0] if targets else (entry * 0.99 if direction == "SHORT" else entry * 1.01)
    target2 = targets[1] if len(targets) > 1 else (target1 * 0.99 if direction == "SHORT" else target1 * 1.01)
