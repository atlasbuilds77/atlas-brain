#!/usr/bin/env python3
"""
TITAN V3 REAL BACKTEST - Polygon Edition
=========================================
Implements the ACTUAL system from Feb 13-14:
1. Sweep + 5-min reclaim rule
2. Multiple sweep filter (2+ in 15 min = skip)
3. 3x+ cluster significance
4. Time-based scaling (1 hour threshold)
5. Trailing stop (+30% → trail at +15%)
6. Max -80% loss
7. Entry at BOUNCE price
"""
...
def find_sweep_reclaim(bars: List[dict], level: float, level_type: str, 
    ...
    Returns sweep info including bounce price (entry point).
    """
    MAX_RECLAIM_BARS = 5
    ...
    for i in range(start_idx, len(bars)):
        ...
        if level_type == "high" and bar['h'] > level:
            # Found sweep above high - look for reclaim (close back below)
            for j in range(i + 1, min(i + 1 + MAX_RECLAIM_BARS, len(bars))):
                if bars[j]['c'] < level:
                    # Entry at the BOUNCE (lowest point after sweep)
                    ...
                    return {
                        ...
                        "reclaim_mins": j - i,
                        "bounce_price": bounce_low,  # Entry point
            ...
            # Found sweep below low - look for reclaim (close back above)
            for j in range(i + 1, min(i + 1 + MAX_RECLAIM_BARS, len(bars))):
                if bars[j]['c'] > level:
                    # Entry at the BOUNCE (highest point after sweep = worst fill for long)
                    # Actually for LONG, bounce_price is the low of the sweep candle
                    bounce_low = min(bars[k]['l'] for k in range(i, j + 1))
                    return {
                        "sweep_idx": i,
                        "sweep_time": bar['time_et'],
                        ...
                        "reclaim_time": bars[j]['time_et'],
                        "reclaim_mins": j - i,
                        "bounce_price": bounce_low,  # Entry point (buy at low)
                        "entry_price": bars[j]['c'],
                        "direction": "LONG"
                    }
            return None
...
# ============================================================
# TRADE SIMULATION WITH REAL EXIT LOGIC
# ============================================================
...
def simulate_trade_real(symbol: str, date: str, direction: str, 
    ...
    for i in range(opt_start_idx, len(option_bars)):
        ...
        if low_gain <= -80:
            ...
            exit_price = option_entry * 0.2  # -80%
            position_open = False
            break
        ...
        # Need to check corresponding underlying bar
...
def analyze_day(symbol: str, date: str, daily_bars: List[dict]) -> Optional[dict]:
    ...
    entry = best_setup['bounce_price']  # Entry at BOUNCE, not reclaim
...
def run_backtest(symbol: str, start: str, end: str):
    """Full backtest with real exit logic."""
    
    print(f"\n{'='*90}")
    print(f"TITAN V3 REAL BACKTEST - Polygon Edition")
    print(f"With: 5-min reclaim, trailing stops, time-based scaling, -80% max loss")
