#!/usr/bin/env python3
"""
Token Monitor with Alerts - Checks session token usage and sends alerts when approaching limits.
"""
...
def send_alert(message, level="WARNING"):
    ...
    try:
        alert_message = f"🚨 TOKEN {level}: {message}"
        log(f"Sending alert: {alert_message}")
        ...
        log(f"Exception sending alert: {e}")
        return False
...
def main():
    """Main function."""
    log("Starting token monitor with alerts...")
    ...
    if critical_alerts:
        alert_message = f"Found {len(critical_alerts)} session(s) at critical token levels (≥{CRITICAL_PERCENTAGE}%). Check {REPORT_FILE}"
        send_alert(alert_message, "CRITICAL")
        for alert in critical_alerts:
            log(f"❌ CRITICAL: {alert['key'][:40]}... at {alert['percentage']}% ({alert['current']:,}/{alert['limit']:,} tokens)")
    
    if warnings:
        alert_message = f"Found {len(warnings)} session(s) approaching token limits (≥{WARNING_THRESHOLD:,} tokens). Check {REPORT_FILE}"
        # Only send alert for warnings if there are no critical alerts (to avoid duplicate alerts)
        if not critical_alerts:
            send_alert(alert_message, "WARNING")
        for alert in warnings:
            log(f"⚠️  WARNING: {alert['key'][:40]}... at {alert['current']:,} tokens (threshold: {WARNING_THRESHOLD:,})")
